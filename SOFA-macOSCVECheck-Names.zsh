...
# Modified by @robjschroeder to report the CVE names into a Jamf Pro EA. 
...
#system_version=15.1 # UNCOMMENT TO TEST OLDER VERSIONS
...
# Define arrays for CVEs and Actively Exploited CVEs
cve_list=()
...
if ! is-at-least "$product_version" "$system_version"; then
    ...
    for j in {0..20}; do
        ...
        if [[ $product_version ]]; then
            cves_in_version=$(/usr/bin/plutil -extract "OSVersions.$os_position.SecurityReleases.$j.CVEs" raw "$json_cache" 2>/dev/null | grep -v "<stdin>")
            if [[ $cves_in_version ]]; then
                for cve in ${(f)cves_in_version}; do
                    #echo "DEBUG: $cve"
                    cve_list+=("$cve")
                done
            fi
...
# Output the lists of CVEs and Actively Exploited CVEs
echo
echo "<result>"
